...
def SaveDataCsv(Data: dict, filename: str):
    if not Data:
        error_logger.error("Data is empty, cannot save to CSV.")
        return
    # Verify if the Data is in the expected format
    if not isinstance(Data, dict):
        error_logger.error("Data is not in the expected format. Expected dictionary.")
        return
    ...
    if not os.path.isfile(filename):
        with open(filename, 'a', newline='') as csvfile:
            try:
                ...
                writer.writeheader()
                info_logger.info(f"File {filename} created")
            except Exception as e:
                error_logger.error(f"Error while creating csv: {e}")
                raise e
    ...
    with open(filename, 'a', newline='') as csvfile:
        try:
            ...
            info_logger.info(f"Data written to {filename}")
        except Exception as e:
            error_logger.error(f"Error while writing to csv: {e}")
            raise e
...
def SupTradingScraperCAC40Company(url: str, header: dict):
    info_logger.info("Starting company's scraper...")
    try:
        Response = requests.get(url, headers=header)
    except Exception as e:
        error_logger.error(f"Request failed: {e}")
    ...
    try:
        ...
        error_logger.error(f"Error while parsing sector: {e}")
    ...
    info_logger.info("Company's scraper finished")
    return DataCompany
